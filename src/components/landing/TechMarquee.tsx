...
const Logos = {
  tailwindcss: () => (
    <div className="flex items-center gap-2">
      ...
      >
        ...
        />
        <path
          className="fill-current text-slate-800 dark:text-slate-100"
          fillRule="evenodd"
          clipRule="evenodd"
  ...
  nextjs: () => (
    <div className="flex items-center gap-1.5 font-bold text-sm sm:text-base">
      <svg
        className="h-4 sm:h-5 w-auto fill-current text-slate-900 dark:text-slate-100"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 394 79"
      >
  ...
  framer: () => (
    <div className="flex items-center gap-1.5 font-bold text-sm sm:text-base text-slate-900 dark:text-slate-100">
      <svg
        viewBox="0 0 14 21"
        role="presentation"
        className="h-4 sm:h-5 w-auto fill-current text-amber-600 dark:text-amber-500"
      >
...
const stackItems = [
  { name: 'Model Context Protocol', icon: <Workflow className="h-4 w-4 text-amber-600 dark:text-amber-500" /> },
  { name: 'LiteLLM Gateway', icon: <Zap className="h-4 w-4 text-orange-500" /> },
  { name: 'Ollama Local AI', icon: <Bot className="h-4 w-4 text-amber-500" /> },
  { name: 'Docker Sandbox', icon: <Container className="h-4 w-4 text-sky-500" /> },
  { name: 'Tailwind CSS', component: Logos.tailwindcss },
  { name: 'Framer Motion', component: Logos.framer },
  { name: 'Next.js', component: Logos.nextjs },
  { name: 'AWS Cloud', component: Logos.aws },
  { name: 'LangGraph Orchestration', icon: <Layers className="h-4 w-4 text-indigo-500" /> },
  { name: 'PostgreSQL & SQLite', icon: <Database className="h-4 w-4 text-emerald-500" /> },
  { name: 'TypeScript & React', icon: <Code2 className="h-4 w-4 text-cyan-500" /> },
  { name: 'Playwright Automation', icon: <Globe className="h-4 w-4 text-rose-500" /> },
  { name: 'Tauri Desktop Runtime', icon: <Terminal className="h-4 w-4 text-purple-500" /> },
];

export const TechMarquee: React.FC = () => {
  return (
    <div
      className="w-full py-2 border-y transition-colors overflow-hidden"
      style={{
    ...
    >
      <div className="max-w-7xl mx-auto px-4 text-center mb-1">
        <p className="text-[10px] sm:text-xs font-mono font-semibold uppercase tracking-widest text-slate-500 dark:text-[#A89B95]">
          POWERED BY INDUSTRY-STANDARD INFRASTRUCTURE &amp; AGENT RUNTIMES
        </p>
      </div>

      <Marquee pauseOnHover speed={35} className="mt-1">
        {stackItems.map((item, index) => {
          if (item.component) {
            ...
            return (
              <div
                key={index}
                className="flex items-center justify-center px-4 py-1.5 rounded-lg border transition-all hover:scale-105 shrink-0"
                style={{
                  borderColor: 'var(--kb-border)',
          ...
          return (
            <div
              key={index}
              className="flex items-center gap-2 px-3.5 py-1.5 rounded-lg border text-xs font-semibold tracking-tight transition-all hover:scale-105 shrink-0"
              style={{
                borderColor: 'var(--kb-border)',
                backgroundColor: 'var(--kb-surface-elevated)',
                color: 'var(--kb-text)',
